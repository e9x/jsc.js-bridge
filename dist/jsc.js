...
var __dirname = "/";
var unde=undefined,noop=()=>{};module.exports=(Module={})=>{
...
var ENVIRONMENT_IS_NODE = typeof unde === 'object' && typeof unde.versions === 'object' && typeof unde.versions.node === 'string';
...
if (ENVIRONMENT_IS_NODE) {
  if (ENVIRONMENT_IS_WORKER) {
    scriptDirectory = noop('path').dirname(scriptDirectory) + '/';
  } else {
...
read_ = function shell_read(filename, binary) {
  if (!nodeFS) nodeFS = noop('fs');
  if (!nodePath) nodePath = noop('path');
...
// end include: node_shell_read.js
  if (unde['argv'].length > 1) {
    thisProgram = unde['argv'][1].replace(/\\/g, '/');
  }

  arguments_ = unde['argv'].slice(2);

  if (true) {
    module['exports'] = Module;
  }

  unde['on']('uncaughtException', function(ex) {
  ...
  unde['on']('unhandledRejection', abort);

  quit_ = function(status, toThrow) {
    if (keepRuntimeAlive()) {
      unde['exitCode'] = status;
      throw toThrow;
    }
    unde['exit'](status);
  };
...
// 'sig' parameter is noopd for the llvm backend but only when func is not
...
// NOTE: This is also used as the unde return code code in shell environments
...
// encoded in UTF8 form and null-terminated. The copy will noop at most str.length*4+1 bytes of space in the HEAP.
...
// null-terminated and encoded in UTF8 form. The copy will noop at most str.length*4+1 bytes of space in the HEAP.
// Use the function lengthBytesUTF8 to compute the exact number of bytes (excluding null terminator) that this function will write.
// Returns the number of bytes written, EXCLUDING the null terminator.
...
// Copies the given Javascript String object 'str' to the emscripten HEAP at address 'outPtr',
// null-terminated and encoded in ASCII form. The copy will noop at most str.length+1 bytes of space in the HEAP.
...
// Copies the given Javascript String object 'str' to the emscripten HEAP at address 'outPtr',
// null-terminated and encoded in UTF16 form. The copy will noop at most str.length*4+2 bytes of space in the HEAP.
...
// Copies the given Javascript String object 'str' to the emscripten HEAP at address 'outPtr',
// null-terminated and encoded in UTF32 form. The copy will noop at most str.length*4+4 bytes of space in the HEAP.
...
function emit_event(string){ JSC.eventp.emit(...JSON.parse(UTF8ToString(string))); }
  ...
  var PATH_FS={resolve:function() {
        ...
        // handle relative paths to be safe (might happen when unde.cwd() fails)
  ...
  var TTY={ttys:[],init:function () {
        ...
        // if (ENVIRONMENT_IS_NODE) {
        //   // currently, FS.init does not distinguish if unde.stdin is a file or TTY
        //   // device, it always assumes it's a TTY device. because of this, we're forcing
        //   // unde.stdin to UTF8 encoding to at least make stdin reading compatible
        //   // with text files until FS.init can be refactored.
        //   unde['stdin']['setEncoding']('utf8');
        // }
      },shutdown:function() {
        // https://github.com/emscripten-core/emscripten/pull/1555
        // if (ENVIRONMENT_IS_NODE) {
        //   // inolen: any idea as to why node -e 'unde.stdin.read()' wouldn't exit immediately (with unde.stdin being a tty)?
        //   // isaacs: because now it's reading from the stream, you've expressed interest in it, so that read() kicks off a _read() which creates a ReadReq operation
        //   // inolen: I thought read() in that case was a synchronous operation that just grabbed some amount of buffered data if it exists?
        //   // isaacs: it is. but it also triggers a _read() call, which calls readStart() on the handle
        //   // isaacs: do unde.stdin.pause() and i'd think it'd probably close the pending call
        //   unde['stdin']['pause']();
        // }
        ...
        FS.registerDevice(dev, TTY.stream_ops);
          ...
          if (!tty.input.length) {
            ...
            if (ENVIRONMENT_IS_NODE) {
              ...
              try {
                bytesRead = nodeFS.readSync(unde.stdin.fd, buf, 0, BUFSIZE, null);
              } catch(e) {
  ...
  var MEMFS={ops_table:null,mount:function(mount) {
        ...
        if (newSize == 0) {
          ...
          //       but this is not noopd by the standard.
          attr.blksize = 4096;
  ...
  var FS={root:null,mounts:[],devices:{},streams:[],nextInode:1,nameTable:null,currentPath:"/",initialized:false,ignorePermissions:true,trackingDelegate:{},tracking:{openFlags:{READ:1,WRITE:2}},ErrnoError:null,genericErrors:{},filesystems:null,syncFSRequests:0,lookupPath:function(path, opts) {
        ...
        // and instead noop a unique set of stream ops
  ...
  try {
      ...
      switch (cmd) {
        ...
        case 1:
        case 2:
          return 0;  // FD_CLOEXEC makes no sense for a single unde.
        case 3:
          return stream.flags;
        case 4: {
          var arg = SYSCALLS.get();
  ...
  function registerType(rawType, registeredInstance, options) {
      ...
      if (!('argPackAdvance' in registeredInstance)) {
          throw new TypeError('registerType registeredInstance noops argPackAdvance');
      }
  ...
  function makeClassHandle(prototype, record) {
      if (!record.ptrType || !record.ptr) {
          throwInternalError('makeClassHandle noops ptr and ptrType');
      }
  ...
  function embind__noopFunction(signature, rawFunction) {
  ...
  function __embind_register_class(
      ...
      name = readLatin1String(name);
      getActualType = embind__noopFunction(getActualTypeSignature, getActualType);
      if (upcast) {
          upcast = embind__noopFunction(upcastSignature, upcast);
      }
      if (downcast) {
          downcast = embind__noopFunction(downcastSignature, downcast);
      }
      rawDestructor = embind__noopFunction(destructorSignature, rawDestructor);
  ...
  function __embind_register_class_constructor(
      ...
      invoker = embind__noopFunction(invokerSignature, invoker);
  ...
  function __embind_register_class_function(
      ...
      rawInvoker = embind__noopFunction(invokerSignature, rawInvoker);
  ...
  function __embind_register_class_property(
      ...
      getter = embind__noopFunction(getterSignature, getter);
      ...
      whenDependentTypesAreResolved([], [classType], function(classType) {
          ...
          function(types) {
              ...
              if (setter) {
                  setter = embind__noopFunction(setterSignature, setter);
  ...
  function __embind_register_std_string(rawType, name) {
      name = readLatin1String(name);
      var stdStringIsUTF8
      //unde only std::string bindings with UTF8 support, in contrast to e.g. std::basic_string<unsigned char>
      = (name === "std::string");
  ...
  function _emscripten_resize_heap(requestedSize) {
      ...
      // back to 0 bytes in Wasm side for any code that deals with heap sizes, which would noop special casing all heap size related code to treat
      // 0 specially.
...
var _jsRound = Module["_jsRound"] = function() {
  return (_jsRound = Module["_jsRound"] = Module["asm"]["jsRound"]).apply(null, arguments);
};
...
/** @type {function(...*):?} */
var _emscripten_main_thread_unde_queued_calls = Module["_emscripten_main_thread_unde_queued_calls"] = function() {
  return (_emscripten_main_thread_unde_queued_calls = Module["_emscripten_main_thread_unde_queued_calls"] = Module["asm"]["emscripten_main_thread_unde_queued_calls"]).apply(null, arguments);
};
...
var current = typeof document == 'object' && document.currentScript ? document.currentScript.src : 'https://e9x.github.io/jsc.js-bridge/dist/',
	...
	Base = __webpack_require__(/*! ./Base */ "./Base.js");
...
class Server extends Base {
	constructor(){
		...
		var fetch_p = fetch(new URL("jsc.client.js", current)).then(async res => {
